...
/* remove the admin bar wo plugin */
add_filter('show_admin_bar', '__return_false');

/* redirect does work w Peter's */
/* Doesn't work w the SimpleModal Login plugin 
function rdo_login_redirect($redirect_to, $request, $user) {
	return if(is_array('administrator', $user->roles)) ? admin_url() : site_url();
	//return site_url();
}
add_filter('login_redirect','rdo_login_redirect', 10, 3); */

/* redirect relating the peter's plugin */
function login_widget_redirect($empty, $redirect_to, $requested_redirect_to, $user) {
	if(stripos($_SERVER['HTTP_REFERER'], '/wp-admin/') !== false) {
		return false;
	} else {
...
/*  */
/*function rediiario_theme_setup() {
	//set_user_setting('dfw_width', 1000);
	add_editor_style(array('style3.css'));
}
add_action('after_setup_theme','rediiario_theme_setup');*/
